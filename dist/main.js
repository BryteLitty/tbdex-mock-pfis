...
function createPFIServer(pfiConfig) {
    ...
    httpApi.onCreateExchange(async (ctx, rfq) => {
        ...
        const payout = (parseFloat(rfq.data.payin.amount) * Number(offering.data.payoutUnitsPerPayinUnit)).toFixed(2);
...
async function updateOrderStatus(rfq, status, pfi, ExchangeRepository) {
...
async function close(rfq, reason, pfi, ExchangeRepository) {
...
myPFIServer5.httpApi.api.get('/vc', async (req, res) => {
    const credentials = await requestCredential(req.query.name, req.query.country, req.query.did);
