...
export async function createExchange(req, res, options) {
    ...
    try {
        ...
        const errorResponse = { detail: `Parsing of TBDex Rfq message failed: ${e.message}` };
    ...
    if (rfqExists) {
        const errorResponse = { detail: `rfq ${rfq.id} already exists` };
        res.status(409).json({ errors: [errorResponse] });
        return;
    }
    const offering = await offeringsApi.getOffering({ id: rfq.data.offeringId });
    if (!offering) {
        const errorResponse = { detail: `offering ${rfq.data.offeringId} does not exist` };
        res.status(400).json({ errors: [errorResponse] });
        return;
    }
    try {
...
function isValidUrl(replyToUrl) {
    try {
        new URL(replyToUrl);
        return true;
    }
